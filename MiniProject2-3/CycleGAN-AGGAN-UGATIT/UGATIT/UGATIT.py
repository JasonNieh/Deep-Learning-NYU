...
class UGATIT(object) :
    def __init__(self, args):
        ...
        print("# dataset_path : ", self.dataset)
    ...
    def train(self):
        ...
        start_iter = 1
        if self.resume:
            model_list = glob(os.path.join(self.result_dir, self.model_name, 'model', '*.pt'))
            if not len(model_list) == 0:
                model_list.sort()
                start_iter = int(model_list[-1].split('_')[-1].split('.')[0])
                self.load(os.path.join(self.result_dir, self.model_name, 'model'), start_iter)
                print(" [*] Load SUCCESS")
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                ...
                cv2.imwrite(os.path.join(self.result_dir, self.model_name, 'img', 'A2B_%07d.png' % step), A2B * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.model_name, 'img', 'B2A_%07d.png' % step), B2A * 255.0)
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()

            if step % self.save_freq == 0:
                self.save(os.path.join(self.result_dir, self.model_name, 'model'), step)
            ...
            if step % 1000 == 0:
                ...
                torch.save(params, os.path.join(self.result_dir, self.model_name + '_params_latest.pt'))
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            ...
            self.load(os.path.join(self.result_dir, self.model_name, 'model'), iter)
            print(" [*] Load SUCCESS")
        else:
            print(" [*] Load FAILURE")
            return
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            cv2.imwrite(os.path.join(self.result_dir, self.model_name, 'test', 'A2B_%d.png' % (n + 1)), A2B * 255.0)
        ...
        for n, (real_B, _) in enumerate(self.testB_loader):
            ...
            cv2.imwrite(os.path.join(self.result_dir, self.model_name, 'test', 'B2A_%d.png' % (n + 1)), B2A * 255.0)
